...
# Friedman test 
friedman <- function(data,alpha){
  if(alpha=="shannon"){
    res.fried <- rstatix::friedman_test(data=data, shannon ~  Stage2 | subject)
    effsz.fried <- rstatix::friedman_effsize(data=data, shannon ~  Stage2 | subject) 
     pwc <- rstatix::wilcox_test(data=data, shannon ~  Stage2 , paired = TRUE, 
                                p.adjust.method = "bonferroni")
    pwc <- pwc %>% rstatix::add_xy_position(x = "Stage2")
    return(list(res.fried=res.fried, effsz.fried=effsz.fried, pwc=pwc))
  }else if(alpha=="simpson"){
    res.fried <- rstatix::friedman_test(data=data, simpson ~  Stage2 | subject)
    effsz.fried <- rstatix::friedman_effsize(data=data, simpson ~  Stage2 | subject) 
    pwc <- rstatix::wilcox_test(data=data, simpson ~  Stage2 , paired = TRUE, 
                                p.adjust.method = "bonferroni")
...
friedman2 <- function(data,value){
  res.fried <- rstatix::friedman_test(data=data, value ~  Stage2 | subject)
  effsz.fried <- rstatix::friedman_effsize(data=data, value ~  Stage2 | subject) 
  pwc <- rstatix::wilcox_test(data=data, value ~  Stage2 , paired = TRUE, 
                      p.adjust.method = "fdr")
  pwc <- pwc %>% rstatix::add_xy_position(x = "Stage2")
  return(list(res.fried=res.fried, effsz.fried=effsz.fried, pwc=pwc))
}
...
sub1 <- function(data0, dis, lev){
  ...
  # remove 90% samples = 0
  temp <- apply(data_MM[,c(9:ncol(data_MM))], 2, function (x) {sum(x<10)})
  temp2 <- temp[which(temp > nrow(data_MM)*0.9)]
  data_MM1 <- data_MM[,-which(colnames(data_MM) %in% names(temp2))]
  data_averaged <- data_MM1
  ...
  data2 <- melt(dcast(melt(data_averaged[,-c(3:8)], ids=c("Stage2","subject")),subject+Stage2~variable))
  data2$id <- paste(data2$subject, data2$Stage2, sep="")
  return(data2)
}

######################### taxon level friedman test
# dis <- "MM","ALL","LM"
# lev <- "p","c","o","f","g","s"
friedman.taxon <- function(data0, dis, lev){
  data2 <-  sub1(data0, dis, lev)
  # friedman test
  friedman.res <- list()
  for (k in unique(data2$variable)){
    ttt <- data2[which(data2$variable==k & data2$subject %in% id),]
    ttt$Stage2 <- factor(ttt$Stage2, levels = c("FCa","FCb", "CRSa","CRSb","CRSc"))
    re<- friedman2(data=ttt, "value")
    friedman.res[[eval(k)]] <- re
  }
  # friedman result summary
  re1 <- lapply(friedman.res, function(x){x$res.fried$p}) %>% unlist() %>% as.data.frame()
  colnames(re1) <- "p"
  re1$p.fdr <- p.adjust(re1$p, method = "fdr")
  re1$p.bonferroni <- p.adjust(re1$p, method = "bonferroni")
  re1$effect.size <- lapply(friedman.res, function(x){x$effsz.fried$effsize}) %>% unlist %>% as.vector
  # pairwise comparisons
  re2 <- lapply(friedman.res, function(x){x$pwc$p.adj}) %>% unlist() %>% matrix(.,ncol=10,byrow = T) %>% as.data.frame()
  rownames(re2) <- names(friedman.res)
  colnames(re2) <- paste(friedman.res[[1]]$pwc$group1, friedman.res[[1]]$pwc$group2, sep="_")
  return(list(re.friedman=re1, re.paired=re2))
}
...
######################### log2fc
fc <- function(data0, dis, lev){
  data2 <-  sub1(data0, dis, lev)
  data3 <- dcast(data2, variable~Stage2, value.var = "value", mean)
  data4 <- data.frame(matrix(nrow=nrow(data3), ncol=10))
  m=1
  for (i in 2:5){
    for (j in (i+1):6){
      k=paste(colnames(data3)[i],colnames(data3)[j], sep="_")
      data4[,m] <- log2(data3[,j]/data3[,i])
      colnames(data4)[m] <- k
      m=m+1
    }
  }
  rownames(data4) <- data3$variable
  return(data4)
}
